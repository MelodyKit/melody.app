        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="login">
            <div>
              ...
              <input type="email" name="email" v-model="loginForm.email" class="bg-neutral-50 border border-neutral-300 text-neutral-900 sm:text-sm font-mono rounded-lg block w-full p-2.5 dark:bg-neutral-700 dark:border-neutral-600 dark:placeholder-neutral-400 dark:text-neutral-50 focus:outline-none" placeholder="email@example.com" required>
            </div>
            <div>
              <label for="password" class="block mb-2 text-neutral-900 dark:text-neutral-50">Password</label>
              <input type="password" name="password" v-model="loginForm.password" class="bg-neutral-50 border border-neutral-300 text-neutral-900 sm:text-sm font-mono rounded-lg block w-full p-2.5 dark:bg-neutral-700 dark:border-neutral-600 dark:placeholder-neutral-400 dark:text-neutral-50 focus:outline-none" placeholder="••••••••" required>
            </div>
            <div>
              <label for="code" class="block mb-2 text-neutral-900 dark:text-neutral-50">Code</label>
              <input type="text" name="code" v-model="loginForm.code" class="bg-neutral-50 border border-neutral-300 text-neutral-900 sm:text-sm font-mono rounded-lg block w-full p-2.5 dark:bg-neutral-700 dark:border-neutral-600 dark:placeholder-neutral-400 dark:text-neutral-50 focus:outline-none" inputmode="numeric" autocomplete="one-time-code">
            </div>
...
import { loginFormIntoData, type LoginForm } from "@/forms/login";
import { useTokensStore } from "@/stores/tokens";

const loginForm: LoginForm = reactive({email: null, password: null, code: null});
...
const login = async () => {
  ...
  const loginData = loginFormIntoData(loginForm);

  await tokensStore.login(loginData);
